...
var readDemand = function readDemand(meter, next) {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      let demand =
        parseInt(reading.recordGroup.record.field.value._text) /
        (DEFAULT_HOURS * dates.day * CHARGE_FACTOR);
...
var parseDate = function (date) {
  ...
  return (
    newDate[2] + newDate[1] + newDate[0] + newDate[3] + newDate[4] + newDate[5]
  );
};
...
var dateFilterSetup = function dateFilterSetup(filter, dates = {}) {
  ...
  switch (filter) {
    case Constants.Meters.filters.today:
      date.begin = parseDate(
        moment().startOf("day").add(6, "hours").format("YYYY-MM-DD HH:mm:ss")
      );
      date.end = parseDate(
        moment().endOf("day").add(6, "hours").format("YYYY-MM-DD HH:mm:ss")
      );
      date.period = 3600;
      return date;
    ...
    case Constants.Meters.filters.week:
      date.begin = parseDate(
        moment().startOf("week").add(6, "hours").format("YYYY-MM-DD HH:mm:ss")
      );
      date.end = parseDate(
        moment().endOf("week").add(6, "hours").format("YYYY-MM-DD HH:mm:ss")
      );
      date.period = 86400;
      return date;
    case Constants.Meters.filters.month:
      date.begin = parseDate(
        moment().startOf("month").add(6, "hours").format("YYYY-MM-DD HH:mm:ss")
      );
      date.end = parseDate(
        moment().endOf("month").add(6, "hours").format("YYYY-MM-DD HH:mm:ss")
      );
      date.period = 86400;
      return date;
    case Constants.Meters.filters.year:
      date.begin = parseDate(
        moment().startOf("year").add(6, "hours").format("YYYY-MM-DD HH:mm:ss")
      );
      date.end = parseDate(
        moment().endOf("year").add(6, "hours").format("YYYY-MM-DD HH:mm:ss")
      );
      date.period = 86400;
      return date;
...
const getCFEGDMTHRateType = function (ISOdate) {
  ...
  // get CFE period
  const period2 = {
    start: Constants.CFE.datePeriods[1].utc_startDate,
    end: Constants.CFE.datePeriods[1].utc_endDate,
  };
  let curr_period = 0;
  if (
    date.isBetween(
      moment(period2.start, "DD/MM/YYYY").tz(timezone),
      moment(period2.end, "DD/MM/YYYY").tz(timezone),
      "days",
      "[]"
    )
  ) {
...
const getCFERate = function (ISOdate, division, tariff_type) {
  ...
  let rate;
  return new Promise((resolve, reject) => {
    AdminValue.findByDate(new_date, division, (err, res) => {
      if (err) reject(err);
      else {
        if (tariff_type === "GDMTO") {
          ...
          return reject(
            "Error, on adminValue.findByDate while getting the rate_type"
          );
        }
