...
const fpReadings = function (
  ...
  service.devices.forEach((device, index) => {
    if (index !== 0) {
      serviceToCall +=
        "?var=" + device.name + ".EPimp" + "?var=" + device.name + ".EQimp";
    }
  });
  ...
  xhr.onerror = function () {
    if (isSingleCompany)
      cb({ status: 500, message: "Error al leer medidor" }, null);
    else cb(null, null);
  };
  xhr.onabort = function () {
    console.log(
      `The request timed out in fpReadings in company ${
...
const monthlyReadings = function (
  meter,
  service,
  isSingleCompany,
  custom_dates,
  ...
  xhr.onload = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      if (reading.recordGroup.record) {
        ...
        let commonFormula =
          consumption / ((dates.hour + dates.day * 24) * CHARGE_FACTOR);
        ...
        Meters.standardReadings(
          ...
          (err, res) => {
            ...
            monthlyReadings.capacity = Math.min(
              maxDpPeak,
              parseFloat(commonFormula)
            );
            monthlyReadings.distribution = Math.min(
              maxDpMonth,
              parseFloat(commonFormula)
            );
  ...
  xhr.onabort = function () {
    console.log(
      `The request timed out in monthly readings in company ${
        meter.company().company_name
      }`
    );
  };
