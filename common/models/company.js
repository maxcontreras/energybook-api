...
module.exports = function (Company) {
  Company.register = function register(contactData, user, cb) {
    Company.findOne({ where: { for_free_trial: true } })
      .then((company) => {
        if (!company)
          return cb({
            status: 404,
            message: "No default company set for free trial",
          });
    ...
    const templatePath = path.join(
      __dirname,
      "/../../server/templates/welcome.mustache"
    );

    mail.sendMail(templatePath, templateVars, {
      to: user.email,
      subject: "El equipo de Energybook te da la bienvenida",
    });
  };

  Company.remoteMethod("register", {
    accepts: [
              ...
              (err, dsgMeter) => {
                if (err) {
                  Meters.destroyById(newMeter.id, () => {
                    return cb({
                      status: 400,
                      message: "Error al crear medidor designado",
                    });
                  });
                } else {
                    ...
                    if (err) {
                      ...
                      Services.setUpBasicService(dsgMeter.id, (err) => {
                        if (err) {
                          Meters.destroyById(newMeter.id, () => {
                            DesignatedMeters.destroyById(dsgMeter.id, () => {
                              return cb({
                                status: 400,
                                message:
                                  "No se pudo crear el servicio para el medidor",
                              });
                            });
                          });
                        } else
                          cb(
                            null,
                            "Medidor " + dsgMeter.id + " asignado correctamente"
                          );
                      });
                    }
                  });
  ...
  Company.remoteMethod("addDesignatedMeter", {
  ...
  Company.remoteMethod("block", {
    accepts: [{ arg: "company_id", type: "string" }],
  ...
  Company.remoteMethod("addAdmins", {
    accepts: [{ arg: "data", type: "object" }],
    returns: { arg: "response", type: "string", root: true },
  });
  ...
  Company.remoteMethod("updateData", {
    accepts: [{ arg: "data", type: "object" }],
    returns: { arg: "response", type: "string", root: true },
  });
