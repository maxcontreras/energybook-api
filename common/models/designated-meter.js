...
module.exports = function (Designatedmeter) {
  Designatedmeter.consumptionSummary = function consumptionSummary(
    company_id,
    cb
  ) {
            ...
            (service, nextService) => {
                  ...
                  if (reading.recordGroup && reading.recordGroup.record) {
                    ...
                    iterable.map((item) => {
                      ...
                      for (let device of itemIterable) {
                        const name =
                          devicesDescription[device.id._text.split(".")[0]];
                    ...
                    service.updateAttribute(
                      ...
                      (err, updated) => {
                        ...
                        Socket.sendMessageToCompanyUsers(
                          company_id,
                          socketData
                        );
                        nextService();
              ...
              xhr.onerror = function () {
                if (meters.length === 1)
                  nextService({
                    status: 500,
                    message: "Error al leer medidor",
                  });
                else nextService();
              };
              xhr.onabort = function () {
                console.error(
                  `The request timed out in consumption summary in company ${
                    meter.company().company_name
                  }`
                );
              };
              xhr.send();
  ...
  Designatedmeter.remoteMethod("consumptionSummary", {
    accepts: [
        ...
        function (meter, next) {
          ...
          async.eachSeries(
            ...
            (service, nextService) => {
              ...
              xhr.onload = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
                  ...
                  if (reading.recordGroup && reading.recordGroup.record) {
                    ...
                    let commonFormula =
                      parseInt(summatory) /
                      (dates.hour * DEFAULT_DAYS * CHARGE_FACTOR);
                    let consumption = summatory;
                    let distributionCharge =
                      commonFormula * Constants.CFE.values.distribution_price;
                    ...
                    Meters.standardReadings(
                      meter.meter_id,
                      "",
                      service.serviceName,
                      Constants.Readings.variables.Demanda,
                      0,
                      900,
                      {},
                      (err, res) => {
                        ...
                        res.forEach((dpReading) => {
                          if (
                            dpReading.isPeak &&
                            parseFloat(dpReading.value) > maxDpPeak
                          ) {
                        ...
                        dailyReadings.capacity = Math.min(
                          maxDpPeak,
                          parseFloat(commonFormula)
                        );
                        dailyReadings.distribution = Math.min(
                          maxDpMonth,
                          parseFloat(commonFormula)
                        );
                        service.updateAttribute(
              ...
              xhr.onabort = function () {
                console.error(
                  `The request timed out in daily readings in company ${
                    meter.company().company_name
                  }`
                );
              };
  ...
  Designatedmeter.remoteMethod("dailyReadings", {
    accepts: [
      { arg: "company_id", type: "string", required: false, default: "" },
    ],
    returns: { arg: "response", type: "string" },
  ...
  Designatedmeter.carbonFootprint = function carbonFootprint(
    company_id,
    service_name,
    cb
  ) {
        ...
        if (service.length > 0) {
          ...
          Promise.all([
            ...
            .then((data) => {
              ...
              cO2Emissions =
                emissionFactor * (parseFloat(consumption) / 1000.0);
              let response = {
                consumption,
                generation,
                total,
  ...
  Designatedmeter.remoteMethod("carbonFootprint", {
    accepts: [
      { arg: "company_id", type: "string", required: false, default: "" },
  ...
  Designatedmeter.generation = function generation(
    company_id,
    service_name,
    device_name,
    cb
    ...
    DesignatedMeter.find({
      ...
      new Promise((resolve, reject) => {
        ...
        .then((devices) => {
          ...
          if (
            designatedMeter["generationDevices"] !== undefined ||
            designatedMeter.generationDevices.length !== 0
          ) {
          ...
          Promise.all([
                    ...
                    async.eachSeries(
                      ...
                      async (item) => {
                        ...
                        const tmp_date =
                          ...
                          second +
                          "Z";
                        const CFE_rates = await EDS.getCFERate(
                          tmp_date,
                          designatedMeter.company().Division,
                          designatedMeter.company().tariff_type
                        );
            ...
            new Promise((resolve, reject) => {
              ...
              dR.getData(designatedMeter.hostname, dates, API_PREFIX, devices, [
                "EPexp",
                "EPimp",
              ])
                .then((reading) => {
  ...
  Designatedmeter.remoteMethod("generation", {
    accepts: [
      { arg: "company_id", type: "string", required: false, default: "" },
      { arg: "service_name", type: "string", required: false, default: "" },
        ...
        function (meter, next) {
          ...
          async.eachSeries(
            ...
            (service, nextService) => {
              ...
              xhr.onabort = function () {
                console.error(
                  `The request timed out in epimpHistory in company ${
                    meter.company().company_name
                  }`
                );
              };
  ...
  Designatedmeter.remoteMethod("epimpHistory", {
    accepts: [
      { arg: "company_id", type: "string", required: false, default: "" },
    ],
    returns: { arg: "response", type: "string" },
                ...
                (err, readings) => {
                  if (err) {
                    ...
                    service.updateAttributes(
                      { fp: readings.fp, reactive: readings.reactive },
                      (err, updated) => {
  ...
  Designatedmeter.remoteMethod("fpReadings", {
    accepts: [
      { arg: "company_id", type: "string", required: false, default: "" },
    ],
    returns: { arg: "response", type: "string" },
  ...
  Designatedmeter.remoteMethod("monthlyReadings", {
    accepts: [
      { arg: "company_id", type: "string", required: false, default: "" },
    ],
    returns: { arg: "response", type: "string" },
            ...
            (service, nextService) => {
              ...
              xhr.onload = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
                  ...
                  let dp = iterable.reduce(
                    (prev, curr) => prev + parseFloat(curr.value._text),
                    0
                  );
              ...
              xhr.onabort = function () {
                console.error(
                  `The request timed out in odometerReadings in company ${
                    meter.company().company_name
                  }`
                );
              };
  ...
  Designatedmeter.remoteMethod("odometerReadings", {
    accepts: [
      { arg: "company_id", type: "string", required: false, default: "" },
    ],
    returns: { arg: "response", type: "string" },
  ...
  Designatedmeter.remoteMethod("getWeather", {
    accepts: [
      { arg: "lat", type: "number" },
      { arg: "lon", type: "number" },
    ],
  ...
  Designatedmeter.setDeviceDescriptions = function setDeviceDescription(
    meterId,
    cb
  ) {
    const DesignatedMeter = app.loopback.getModel("DesignatedMeter");
      ...
      async.each(
        ...
        (err) => {
          if (err)
            return cb({
              status: 501,
              message: "Error al guardar descripción de un dispositivo",
            });
          cb(null, "OK");
        }
      );
  ...
  Designatedmeter.remoteMethod("setDeviceDescriptions", {
    accepts: [{ arg: "meterId", type: "string", required: false }],
    returns: { arg: "result", type: "string" },
  });

  Designatedmeter.deleteMeterWithServices = function deleteMeterWithServices(
    meterId,
    cb
  ) {
    const Services = app.loopback.getModel("Service");

    Designatedmeter.findById(
      meterId,
      { include: ["meter", "services"] },
      (err, dsgMeter) => {
  ...
  Designatedmeter.remoteMethod("deleteMeterWithServices", {
    accepts: [{ arg: "meterId", type: "string" }],
    returns: { arg: "result", type: "string" },
  });
  ...
  Designatedmeter.upload = function (ctx, options, cb) {
    ...
    Designatedmeter.app.models.container.upload(
      ctx.req,
      ctx.result,
      options,
      function (err, fileObj) {
        if (err) {
          ...
          for (var k in fileObj.files) {
            //  console.log(k, fileObj.files[k]);
            var name = fileObj.files[k][0].name;
          }

          // console.log(name);
          /*
          var fileInfo = fileObj.files.options;
          ...
          */
          ...
          fs
            .createReadStream(
              "../energybook-api/ArchivosTemporales/common/" + name
            ) //Ruta dinamica a ArchivosTemporales/common/
            .pipe(csv.parse())
            .on("data", (row) => {
              //por cada columna
              if (row[0] == "Time") {
            ...
            .on("end", function () {
              ...
              // Importa el modelo que necesita
              let Fecha = valores[0].split(" ");
              // Divide fecha para sacar hora y dia

              // console.log(Fecha[1]);

              var Arreglo = {
                V12: parseInt(valores[5]),
                V23: parseInt(valores[7]),
                V31: parseInt(valores[6]),
                I1: parseInt(valores[9]),
                I2: parseInt(valores[10]),
                I3: parseInt(valores[11]),
                Ssum_KVA: parseInt(valores[25]),
                PF1: parseInt(valores[26]),
                PF2: parseInt(valores[27]),
                PF3: parseInt(valores[28]),
                EP_IMP_kWh: parseInt(valores[35]),
                EP_EXP_kWh: parseInt(valores[39]),
                DMD_P_kW: parseInt(valores[56]),
                Unbl_U: parseInt(valores[62]),
                Unbl_I: parseInt(valores[63]),
                THD_Ia: parseInt(valores[68]),
                THD_Ib: parseInt(valores[69]),
                THD_Ic: parseInt(valores[70]),
              };

              var string = "horaaaa";
              var varArreglo = Fecha[1];

              var obj = {};
              obj["DesignatedMeterId"] = DesignatedMeterid;
              obj["Dia"] = Fecha[0];
              obj[varArreglo] = Arreglo;

              Minutes.upsertWithWhere(
                { Dia: Fecha[0] }, // aqui añadir despues division del id de empresa y de medidor
                obj
              ).then((ok) => {
            ...
            fs.unlink(
              "../energybook-api/ArchivosTemporales/common/" + name,
              (err) => {
                // Lo que hace esto elimina el archivo temporal
                if (err) throw err;
  ...
  Designatedmeter.remoteMethod("upload", {
